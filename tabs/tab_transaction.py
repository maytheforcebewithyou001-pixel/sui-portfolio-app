...
def _parse_sbi_csv(csv_file):
    """SBI証券 約定履歴CSVをパースしDataFrameで返す"""
    import io
    raw = csv_file.read()
    csv_text = None
    ...
    if csv_text is None: return None, "ファイルのエンコーディングを判別できませんでした。"

    lines = csv_text.splitlines()
    header_idx = None
    for i, line in enumerate(lines):
        if "約定日" in line and "銘柄" in line: header_idx = i; break
    if header_idx is None: return None, "SBI約定履歴のヘッダー行が見つかりませんでした。"
    ...
    def _tax(v):
        ...
        return "特定口座"
    csv_df["_口座区分"] = csv_df["預り"].apply(_tax)
...
def render(tab, df):
    with tab:
        ...
        st.caption("SBI証券の約定履歴CSV（口座管理→取引履歴→CSV出力）を取り込みます。")
        csv_file = st.file_uploader("CSVファイルを選択", type=["csv"], key="csvup")
        if csv_file:
            csv_df, err = _parse_sbi_csv(csv_file)
            ...
            if st.button("✅ インポート実行", use_container_width=True, key="csvimport"):
                ...
                if imp_mode in ("保有銘柄の数量を更新", "両方（取引履歴＋保有銘柄更新）"):
                    for _, crow in csv_df.iterrows():
                        ...
                        qty, price = float(crow["約定数量"]), float(crow["約定単価"])
