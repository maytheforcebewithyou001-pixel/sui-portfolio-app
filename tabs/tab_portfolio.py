...
def render(tab, df, display_df, totals):
    ...
    with tab:
        ...
        if not df.empty and not display_df.empty:
            _render_account_summary(df, display_df)
            sel = _render_holdings_table(display_df)
            _render_stock_detail(display_df, sel)
            _render_export(display_df)
        if not df.empty:
            _render_editor(df)
        if TA > 0:
            _render_history_chart(df, TA)


def _render_add_form(df):
    # ── 銘柄追加フォーム ──
    st.markdown("#### ➕ 銘柄を追加")
...
def _render_account_summary(df, display_df):
    # ── 口座別サマリー ──
...
def _render_holdings_table(display_df):
    # ── 保有一覧 ──
...
def _render_stock_detail(display_df, sel):
    # ── 銘柄詳細 ──
...
def _render_history_chart(df, TA):
    # ── 資産推移チャート ──
